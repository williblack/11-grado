  ...
  if(C.tarea){
    ...
    H += seccion('tarea','b-tarea','📄 Tarea en hoja suelta', t.titulo,
      `${t.indicacion ? `<div class="flecha-izq"><span class="fl">⬅️</span> ${esc(t.indicacion)}</div>` : ''}
       <div class="papel"><b>${fmt(t.instruccion)}</b>${Array.from({length: t.renglones == null ? 5 : t.renglones}).map(()=>'<div class="renglon"></div>').join('')}</div>
...
/* ===================== ARRANQUE (con reparación automática) ===================== */
function repararLlavesBernal(src){
  var pila = [], out = src.split(""), arreglos = [], linea = 1, q = null, esc = false, com = null;
  var par = {"{":"}", "[":"]", "(":")"};
  for(var i = 0; i < src.length; i++){
    var c = src[i];
    if(c === "\n"){ linea++; if(com === "//") com = null; continue; }
    if(com === "//") continue;
    if(com === "/*"){ if(c === "*" && src[i+1] === "/"){ com = null; i++; } continue; }
    if(q){ if(esc){ esc = false; continue; } if(c === "\\"){ esc = true; continue; } if(c === q) q = null; continue; }
    if(c === '"' || c === "'" || c === "`"){ q = c; continue; }
    if(c === "/" && src[i+1] === "/"){ com = "//"; continue; }
    if(c === "/" && src[i+1] === "*"){ com = "/*"; i++; continue; }
    if(par[c]){ pila.push(c); continue; }
    if(c === "}" || c === "]" || c === ")"){
      var top = pila[pila.length-1];
      if(top && par[top] !== c){ out[i] = par[top]; arreglos.push(linea); }
      pila.pop();
    }
  }
  return {codigo: out.join(""), lineas: arreglos};
}
function avisoBernal(html){
  var d = document.createElement("div");
  d.style.cssText = "position:fixed;left:12px;right:12px;bottom:12px;z-index:400;background:#fef3c7;color:#78350f;border:2px solid #f59e0b;border-radius:14px;padding:12px 44px 12px 16px;font:600 14px system-ui,sans-serif;box-shadow:0 10px 30px -10px rgba(0,0,0,.4)";
  d.innerHTML = html + '<button style="position:absolute;right:10px;top:8px;border:0;background:none;font-size:20px;cursor:pointer" onclick="this.parentNode.remove()">✕</button>';
  document.body.appendChild(d);
}
function errorBernal(html){
  document.body.innerHTML = '<div style="font:17px/1.6 system-ui,sans-serif;max-width:760px;margin:40px auto;padding:24px;border:3px solid #dc2626;border-radius:18px;background:#fff5f5;color:#450a0a">' + html +
    '<p style="margin-top:16px">🔧 <b>Solución rápida:</b> abre el <a href="../../armador.html">armador</a>, pega el código de este index.html y genera uno nuevo. ' +
    'O pídele a la IA: <i>«Revisa comas, comillas, llaves { } y corchetes [ ] y dame de nuevo el archivo completo»</i>.</p></div>';
}
(function(){
  if(CLASE_BERNAL){ __montarBernal(); __motorBernal(); return; }
  if(location.protocol === "file:"){ errorBernal("⚠️ <b>El bloque const CLASE tiene un error de escritura</b> o no existe en este index.html."); return; }
  fetch(location.href, {cache: "no-store"}).then(function(r){ return r.text(); }).then(function(html){
    var k = html.search(/const\s+CLASE\s*=/);
    if(k < 0){ errorBernal("⚠️ <b>Falta el bloque <code>const CLASE = {...}</code></b> en este index.html."); return; }
    var lineaBase = html.slice(0, k).split("\n").length;
    var fin = html.indexOf("</" + "script>", k);
    var src = html.slice(k, fin < 0 ? html.length : fin).replace(/^const\s+CLASE\s*=\s*/, "").trim().replace(/;\s*$/, "");
    var r = repararLlavesBernal(src), obj = null, msg = "";
    try { obj = (new Function("return (" + r.codigo + ");"))(); } catch(e){ msg = e.message; }
    var lineas = r.lineas.map(function(n){ return n + lineaBase - 1; });
    if(obj){
      CLASE_BERNAL = obj; __montarBernal(); __motorBernal();
      avisoBernal("⚠️ <b>Aviso para el profe:</b> este index.html tenía " + lineas.length + " error(es) de llaves/corchetes" + (lineas.length ? " en la(s) línea(s) " + lineas.join(", ") : "") +
        ". La página los corrigió sola para mostrarse, pero conviene arreglar el archivo en GitHub (o pasarlo por el armador).");
    } else {
      errorBernal("⚠️ <b>El bloque CLASE tiene un error de escritura</b> que no se pudo corregir automáticamente.<br>Detalle: <code>" + String(msg).replace(/</g,"&lt;") + "</code>" +
        (lineas.length ? "<br>Revisa cerca de la(s) línea(s): <b>" + lineas.join(", ") + "</b>" : ""));
    }
  }).catch(function(){ errorBernal("⚠️ <b>El bloque const CLASE tiene un error de escritura</b> en este index.html."); });
})();
